...
class Segmenter:
    def __init__(self, ellipsoid_list, camera_list):
        self.el = ellipsoid_list
        self.cl = camera_list

    def labeling(self, camera_id_range, save_path, resolution_divider=100, opacity_threshold=0.9, load_path=""):
        if load_path == "":
            label_lists = [[] for _ in range(self.el.size)]
        else:
        ...
        # OPACITY FILTER
        eo = EllipsoidList(ellipsoid_list=self.el.get_ellipsoids_above_opacity_lvl(opacity_threshold))

        camera_counter = 1
        for camera_idx in camera_id_range:
            cam = self.cl.cameras[camera_idx]
            ...
            for i in range(0, 720):
                for j in range(0, 1280):
                    ...
                    if i % divider == 0 and j % divider == 0:
                        ...
                        label = self.assign_label(cam.image.image[i, j])
    ...
    def assign_label(self, color):
        if np.sum(np.array([107, 142, 35])) == np.sum(color):
            return 1  # color_vegetation
        elif np.sum(np.array([0, 0, 0])) == np.sum(color):
            return 2  # color_statue_and_sky
        elif np.sum(np.array([153, 153, 153])) == np.sum(color):
            return 3  # color_pole
        elif np.sum(np.array([102, 102, 156])) == np.sum(color):
            return 4  # color_wall
        elif np.sum(np.array([70, 70, 70])) == np.sum(color):
            return 5  # color_building
        elif np.sum(np.array([0, 0, 255])) == np.sum(color):
            return 6  # color_vehicle
        elif np.sum(np.array([244, 35, 232])) == np.sum(color):
            return 7  # color_sidewalk
        elif np.sum(np.array([128, 64, 128])) == np.sum(color):
            return 8  # color_road
        elif np.sum(np.array([157, 234, 50])) == np.sum(color):
            return 9  # color_roadline
        elif np.sum(np.array([190, 153, 153])) == np.sum(color):
            return 10  # color_fence
        else:
            return -1
...
if __name__ == '__main__':
    ...
    segmenter = Segmenter(ellipsoid_list=el, camera_list=cl)
    ...
    segmenter.labeling(cd_index_range, save_path, resolution_divider=res_div, load_path=load_path)